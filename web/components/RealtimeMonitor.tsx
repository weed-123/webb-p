...
export default function RealtimeMonitor() {
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      setLatestData(data);
      setDebug(`Data refreshed at ${new Date().toLocaleTimeString()}: ${data ? "Data received" : "No data found"}`);
      setLoading(false);
    } catch (err) {
  ...
  useEffect(() => {
    setDebug("Setting up Firebase listener...");
    const latestRef = ref(db, 'latest');
    console.log("Setting up Firebase listener for 'latest' path");
    
    try {
      const unsubscribe = onValue(latestRef, 
        (snapshot) => {
          const data = snapshot.val();
          console.log("Data received from Firebase:", data);
          setLatestData(data);
          setDebug(`Real-time update at ${new Date().toLocaleTimeString()}: ${data ? "Data received" : "No data found"}`);
          setLoading(false);
        },
        (error) => {
          console.error("Firebase error:", error);
          setError(error.message);
          setDebug(`Listener error at ${new Date().toLocaleTimeString()}: ${error.message}`);
          setLoading(false);
        }
      );
      ...
      return () => {
        unsubscribe();
        setDebug("Firebase listener removed");
      };
    } catch (err) {
      console.error("Error setting up Firebase listener:", err);
      const errorMsg = err instanceof Error ? err.message : String(err);
      setError(errorMsg);
      setDebug(`Setup error at ${new Date().toLocaleTimeString()}: ${errorMsg}`);
      setLoading(false);
      return () => {};
    }
  }, []);
